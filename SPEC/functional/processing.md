...
- В процессе формирования форматов, происходит пред-пост-обработка, частично описано в cleanup.md, но в коде намного больше логики
- Потом форматы храним в кэше: cache.md
- Форматы markdownFile и htmlFile не формируются в момент snapshot. Их генерируем из markdown и outerHTML:
   - Когда пользователь нажимает кнопку на странице COPIED: ../pages/copied.md
   - Если это дефолтное действие: ../pages/settings.md
