# ДОСТУПНЫЕ ФОРМАТЫ

---

## Devtools
- code
- selector
- JS path
- XPath
- full XPath
- styles
- computed styles

---

## Форматы Copy
- Text
   - text/html основной
   - text/plain вычисляется из text/html
   - Человекочитаемый текст с форматированием для GoogleDocs / Word
- Markdown
   - headingStyle atx
   - hr `---`
   - fence ```
   - bullet list `-`
   - **Bold**, *italic*, ~~strikethrough~~
   - link style [title](link)
- Image

---

## Форматы Download
- Это файлы для скачивания
- Название файла в формате `C-D-T.E`, где:
   - C = `copied`
   - D = домен сайта, точки меняем на дефис
   - T = название тега
   - E = расширение файла
- Список:
   - Markdown // Формируется из текста markdown
   - PNG
   - JPEG
   - HTML // Формируется из текста code

---

## URL
- URL

---

## СВЯЗАННЫЕ СТРАНИЦЫ UI
- COPIED -- информация о сохранении / копировании, кнопки действия
- SETTINGS -- пользовательские настройки

---

## ДЛЯ ПРОИЗВОДНЫХ ФОРМАТОВ

- Прежде чем формировать форматы (Text, Markdown, Image, PNG, JPEG) необходимо вырезить noscript и теги комментариев `<!--...-->`
- Данное правило не применяется для форматов Devtools
